...
async function firecrawlFetch(
	...
	const meta = (("data" in json && json.data?.metadata) ||
		("metadata" in json && json.metadata) ||
	...
	const status =
		typeof meta.statusCode === "number" ? (meta.statusCode as number) : 200;
	const finalUrl =
		typeof meta.sourceURL === "string" ? (meta.sourceURL as string) : url;
