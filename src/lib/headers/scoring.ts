...
export function computeScore(findings: HeaderFinding[]): { score: number; level: AnalysisReport["level"]; summary: string } {
  ...
  const level: AnalysisReport["level"] =
    score >= 90 ? "excellent" : score >= 70 ? "acceptable" : score >= 40 ? "deficient" : "critical";
